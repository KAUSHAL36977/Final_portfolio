...
export default function HUDOverlay() {
  const { mode, audioEnabled, setMode, toggleAudio } = useExperienceStore();
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-40 p-6 flex flex-col justify-between font-mono text-xs md:text-sm text-os-cyan/80 uppercase">
      ...
      <div className="flex justify-between items-start">
        ...
        >
          <div className="pointer-events-auto flex gap-4">
            ...
            <button
              onClick={() => setMode(mode === 'playable' ? 'cinematic' : 'playable')}
              className="hover:text-white transition-colors"
            >
      ...
      {/* Crosshair (Center) - Only show in playable mode */}
      {mode === 'playable' && (
      ...
      <div className="flex justify-between items-end">
        ...
        >
          {mode === 'playable' ? (
            ...
            <div className="flex gap-2 items-center">
              <div className="ml-2">SCROLL TO EXPLORE</div>
            </div>
          )}
        </motion.div>
